% =========================================================================
%  Dados experimentais do LEA BCS   
% =========================================================================
function Cexp = dados_LEA_Exp(LEA_BCS_Instrumentacao,Tinicio,Ts,Tfim,tam)

LEA_BCS_Instrumentacao = LEA_BCS_Instrumentacao(Tinicio:Ts:Tfim,1:tam);
...
Cexp.valvula_pneumatica_topo = double(((LEA_BCS_Instrumentacao(:,50))));          % [%]    Valor da válvula pneumática de controle na cabeça do poço
...
Cexp.vazao_producao = double(LEA_BCS_Instrumentacao(:,13));                                % [m³/h] Vazão na choke
...
Cexp.pressao_intake = double((LEA_BCS_Instrumentacao(:,32)))-1-8.6*855*9.81/10^5; % [Bar]  Pressao de intake da bomba, colhido no inversor, há duas correções necessárias: 1 - A pressão deve ser subtraída da atmosférica; 2- é necessário corrigir a posição da entrada da bomba, portanto retirar o peso da coluna de líquido.
...
Cexp.densidade = double((LEA_BCS_Instrumentacao(:,39)));               % [ºC]    Temperatura do coriolis obtida da vazão de reservátorio
Cexp.IP = (Cexp.vazao_reservatorio./3600)./((Cexp.pressao_reservatorio*1e05 - Cexp.pressao_fundo*1e05));               % [ºC]    Temperatura do coriolis obtida da vazão de reservátorio
Cexp.visc = (30.242*exp(-0.024*Cexp.temperatura_intake))/1000;               % [ºC]    Temperatura do coriolis obtida da vazão de reservátorio
Cexp.Head = double((LEA_BCS_Instrumentacao(:,39)));               % [ºC]    Temperatura do coriolis obtida da vazão de reservátorio
